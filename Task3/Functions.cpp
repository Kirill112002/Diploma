...
static string adding_new_xor_variable(int newLet, int let1, int let2)//equi(ââåäåíèå íîâîé ïåğåìåííîé) ğàáîòàåò çà êîíñòàíòó, íå òğåáóåò remake
{
	string equi = "[";
...
string res_to_Dimacs(string res)
{
	long long counter = 0;
	stringstream ss(" " + res);
	string conjunñt, disjunct, ans = "";
	while (!ss.eof()) {
		getline(ss, conjunñt, ']');
		if (conjunñt != "")
		{
			conjunñt = conjunñt.substr(3, conjunñt.length() - 3);
			stringstream sc(conjunñt);
